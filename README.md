...
팝업을 닫았다 다시 열어도 직전 입력·답변이 유지된다(브라우저 재시작 시 비워짐). 다시 열면 입력창의 기존 텍스트가 **전체 선택**된 채로 떠서, 바로 타이핑하면 새 단어로 교체된다(이어쓰려면 → 또는 클릭으로 선택 해제). 새 단어를 묻고 싶으면 `물어보기` 옆 `🔄 새로고침`으로 입력·답변을 비워 빈 입력창으로 초기화한다. `📋 복사`는 답변이 있을 때 입력창 아래에 뜬다(답변은 자체 스크롤이라 길어도 밀리지 않는다). 설정은 자주 쓰지 않으므로 별도 버튼 없이 **확장 아이콘 우클릭 → "옵션"** 으로 연다.
