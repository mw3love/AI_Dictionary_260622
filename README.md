...
2. 전역 단축키(기본 미설정 — `chrome://extensions/shortcuts`에서 지정) 또는 확장 아이콘 클릭으로 현재 페이지 위에 오버레이 패널을 연다(같은 키/클릭으로 닫힘). *예전 기본값 `Alt+Q`는 youtube_dual_subtitle 확장의 "자막 직접 질문"에 양보 — YouTube에서 동시 발화를 피하려 기본 단축키를 비웠고, 원하는 키를 직접 지정한다.* 패널은 항상 화면 오른쪽에 뜨며 **유튜브 등 HTML5 전체화면 위에서도** 같은 위치를 유지한다. 헤더 바를 드래그해 위치 이동, `–`로 📖 아이콘 최소화(다시 클릭해 복원), `✕`(또는 `Esc`)로 닫는다.
3. 입력창에 `Ctrl+V`로 붙여넣거나 직접 입력하고 Enter → AI 답변.
4. (선택) `🖍 형광펜`을 켜고 답변에서 **중요한 부분을 드래그 선택**하면 빨강 강조(단축키 `Shift+백틱`). 형광펜 모드에서 친 부분을 **클릭하면 개별 해제**(모드가 아닐 땐 클릭 무시 — 드래그로 텍스트 선택만 됨), `↺`로 **사람이 친 형광펜만 전체 해제**(모델이 쓴 코드는 보존). 형광펜은 탭마다 따로 보관되며 복사·Notion 저장에 그대로 반영.
